...
def run_madgraph(proc_card_path, output_path, ufo_path, param_card_path=None, run_card_path=None, n_events=None):
    ...
    run_dir = os.path.join(os.cwd(), 'PROC_sm_0', 'Events', 'run_1')
    subprocess.call(['gunzip', os.path.join(run_dir, 'tag_1_pythia8_events.hepmc.gz')])
    subprocess.call(['sudo', 'cp', os.path.join(run_dir, 'tag_1_pythia8_events.hepmc'), output_path])
...
def main():
    ...
    parser.add_argument('--param_card', help='path to param_card. Leave unspecified or pass "default" to use the default param card for the model.')
    parser.add_argument('--run_card', help='path to run_card. Leave unspecified or pass "default" to use the default run card for the model.')
    parser.add_argument('--n_events', '-n', type=int, help='Number of events.')

    args = parser.parse_args()
    run_madgraph(args.proc_card, args.output, args.ufo, args.param_card, args.run_card, args.n_events)
