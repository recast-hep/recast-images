...
def edit_proc_card(proc_card_path, param_card_path, run_card_path, n_events):
    with open(proc_card_path, 'a') as f:
        ...
        if n_events is not None:
            f.write('set nevents {}'.format(n_events))


def run_madgraph(output_path, ufo_path, proc_card_path, param_card_path=None, run_card_path=None, n_events=None):
    ...
    subprocess.run(['/code/madgraph/bin/mg5_aMC', proc_card_copy_path])
...
def main():
    ...
    run_madgraph(args.output, args.ufo, args.proc_card, args.param_card, args.run_card, args.n_events)
